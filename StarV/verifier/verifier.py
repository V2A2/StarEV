...
from StarV.net.network import NeuralNetwork, reachExactBFS, reachExactBFS_Rnn
...
def quantiVerifyExactBFS_Rnn(net, inputSet, unsafe_mat, unsafe_vec, lp_solver="gurobi", numCores=1, show=True):
    """Quantitative Verification of ReLU Networks using exact bread-first-search"""
    ...
    if numCores > 1:
        ...
        pool = None
    S = reachExactBFS_Rnn(net, inputSet, lp_solver, pool, show)  # output set
    P = []  # unsafe output set
    prob = []  # probability of unsafe output set
    if pool is None:
        for Sn in S:
...
def quantiVerifyBFS(net, inputSet, unsafe_mat, unsafe_vec, p_filter=0.0, lp_solver="gurobi", numCores=1, show=True):
    ...
    if p_filter == 0.0 and ("rnn" in net.type):
        S, P, p_v = quantiVerifyExactBFS_Rnn(net, inputSet, unsafe_mat, unsafe_vec, lp_solver, numCores, show)
        p_v_ub = p_v
        p_v_lb = p_v
